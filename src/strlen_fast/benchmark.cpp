...
void strlen_benchmark_fixed_string(int str_len, int iterations)
{
...

    {
        ...
        printf("%20s = %7" PRIuPTR ", time spent: %8.3f ms, sum: %" PRIuPTR "\n",
...

    {
        ...
        sw.start();
        for (int i = 0; i < iterations; ++i) {
            len = ::strlen_fast_v1(str);
            sum += len;
        }
        sw.stop();

        printf("%20s = %7" PRIuPTR ", time spent: %8.3f ms, sum: %" PRIuPTR "\n",
            "strlen_fast_v1(str)", strlen_fast_v1(str), sw.getMillisec(), sum);
    }

    {
        ...
        sw.start();
        for (int i = 0; i < iterations; ++i) {
            len = ::strlen_fast_v2(str);
            sum += len;
        }
        sw.stop();

        printf("%20s = %7" PRIuPTR ", time spent: %8.3f ms, sum: %" PRIuPTR "\n",
            "strlen_fast_v2(str)", strlen_fast_v2(str), sw.getMillisec(), sum);
    }
...
void strlen_benchmark()
{
    ...
    static int s_max_iterations_0[] = { 96000, 80000, 72000, 64000, 56000, 48000, 40000, 36000, 32000, 28000, 24000,
                                        20000, 16000, 12000, 10000, 8000, 6000, 5500, 5000, 4000, 3000, 0 };
    length = 1;
    for (int i = 0; i < 22; ++i) {
    ...
    static int s_max_iterations_2[] = { 16000, 16000, 16000, 8000, 8000, 4000, 2000, 1000, 500, 250, 0 };
    length = (1 << 4);
    for (int i = 0; i < 10; ++i) {
